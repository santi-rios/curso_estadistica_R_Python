...
Escribe el código de R necesario para crear los valores en y de la distribución normal de arriba usando la función `dnorm()`. Cuando estés listo, haz click en el botón _"Run Code"_ para ver el gráfico. Recuerda que `dnorm()` toma como argumentos el vector de valores (en este caso, `x`), la media y la desviación estándar.
