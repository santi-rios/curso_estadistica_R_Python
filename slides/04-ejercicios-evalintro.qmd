...
- Ahora que ya tenemos los datos limpios, podemos explorarlos.
- Nos interesa ver la cantidad de casos de tuberculosis por país, año y sexo.
- Para ir tratando de hacer más agil el análisis, hay que acostumbrarnos a trabajar con las pipas `%>%` y a encadenar funciones. Si todavía te cuesta, puedes seguir haciendo pequeños pasos y verificando que todo esté bien. 
- ¿Cómo podríamos empezar a explorar la cantidad de casos de tuberculosis por país, año y sexo?
- Primero, vamos a ver un resumen de los datos agrupados por país, año y sexo. Después, vamos a ver un gráfico que nos muestre la cantidad de casos de tuberculosis.
- Primero, ¿Qué función de `dplyr` te permitiría hacer esto?

```{webr}
#| exercise: examen1_10

casos_agrupados <- oms_limpio %>%
  ______(______) %>% # Agrupa los datos por país, año y sexo
  ______(______) %>% # Suma los casos de tuberculosis
  ______(______) # Ordena los datos por numero de casos

casos_agrupados # Verificamos que se haya hecho correctamente

```

```r	
oms_grupo <- oms_limpio %>%
  group_by(pais, anio, sexo) %>%
  summarise(ncasos = sum(valor), .groups = 'drop') %>%
  arrange(desc(ncasos))
```

- Ahora, vamos a visualizar los datos.
- Para esto, vamos a usar la función `ggplot()` del paquete `ggplot2`.
- Qué tipo de gráfico te permitiría visualizar la cantidad de casos de tuberculosis por país, año y sexo?
- Piensa en cómo podrías hacer esto. ¿Qué variables usarías en los ejes `x` y `y`? ¿Qué variable usarías para el color? ¿qué tipo de variables son estas?
  - En el eje `x`, podrías poner el año.
  - En el eje `y`, podrías poner la cantidad de casos.
  - Para el color, podrías poner el sexo.

```{webr}
#| exercise: examen1_11

library(ggplot2) # Carga la librería necesaria

ggplot(data = casos_agrupados, aes(x = anio, y = ncasos, color = sexo)) +
  geom_line() +
  geom_point() +
  labs(title = "Casos de tuberculosis por país, año y sexo",
       x = "Año",
       y = "Número de casos")
```
...
ggplot(data = oms_grupo, aes(x = anio, y = ncasos, color = sexo)) +
  geom_line() +
  geom_point() +
  labs(title = "Casos de tuberculosis por país, año y sexo",
       x = "Año",
...
ggplot(oms_grupo, aes(x = reorder(pais, -ncasos), y = ncasos, fill = pais)) +
  geom_bar(stat = "identity") +
  labs(title = "Casos por País",
       x = "País",
       y = "Número de Casos") +
  theme_minimal()
...
ggplot(oms_grupo, aes(x = anio, y = ncasos, color = pais)) +
  geom_line() +
  geom_point() +
  labs(title = "Casos por País",
       x = "País",
...
oms_grupo %>%
    group_by(pais, anio) %>%
    summarise(casos_propmedio = mean(ncasos), .groups = 'drop') %>%
    ggplot(aes(x = reorder(pais, -casos_propmedio), y = casos_propmedio, fill = pais)) +
