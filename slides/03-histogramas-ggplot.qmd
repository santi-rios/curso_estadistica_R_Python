...
- Siguiendo la estructura básica que ya conocemos, vamos a crear un histograma con los datos de las jirafas.
- Nota que ahora usam `binwidth = 1` en `geom_histogram()` para especificar el ancho de las barras del histograma. Puedes experimentar con diferentes valores para ver cómo cambia el gráfico.
...
Agregar títulos y etiquetas al gráfico. Nota que estamos utilizando el objeto `p` que creamos anteriormente. Seguimos agregando capas al gráfico con el operador `+` y lo seguimos guardando en el objeto `p` para futuras modificaciones. Para mostrar el gráfico, simplemente llamamos al objeto `p` o bien, puedes usar `print(p)`.
